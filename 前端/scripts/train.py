"""
Train multiple classification models (Logistic Regression, Ridge, Lasso, RandomForest, XGBoost)
to predict school-level AP/IB proficiency (binary classification).
Outputs classification metrics table during model building and ROC curves + metrics table during evaluation.
"""
...
try:
    ...
    raise ImportError(
        "xgboost 库未安装，请先运行 `pip install -r requirements.txt`。"
    ) from exc
...
def load_level4_data() -> pd.DataFrame:
    if not LEVEL4_PATH.exists():
        raise FileNotFoundError(
            ...
            f"请确保数据文件存在于该路径，或修改脚本中的 DATA_ROOT 路径。"
        )
...
def save_results(df: pd.DataFrame) -> None:
    ...
    for _, row in df.iterrows():
        acc_str = f"**{row['accuracy_score']:.2f}**" if abs(row['accuracy_score'] - best_acc) < 0.001 else f"{row['accuracy_score']:.2f}"
        rec_str = f"**{row['recall_score']:.2f}**" if abs(row['recall_score'] - best_rec) < 0.001 else f"{row['recall_score']:.2f}"
        prec_str = f"**{row['precision']:.2f}**" if abs(row['precision'] - best_prec) < 0.001 else f"{row['precision']:.2f}"
        f1_str = f"**{row['f1_score']:.2f}**" if abs(row['f1_score'] - best_f1) < 0.001 else f"{row['f1_score']:.2f}"
        auc_str = f"**{row['area_under_curve']:.3f}**" if abs(row['area_under_curve'] - best_auc) < 0.001 else f"{row['area_under_curve']:.3f}"
...
def create_metrics_table_building(df_results: pd.DataFrame) -> None:
    """创建建立模型阶段的性能指标表格图"""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    fig_path = OUTPUT_DIR / "classification_metrics_building.png"
...
def create_metrics_table_evaluation(df_results: pd.DataFrame) -> None:
    """创建评估阶段的性能指标表格图"""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    fig_path = OUTPUT_DIR / "classification_metrics_evaluation.png"
...
def main() -> None:
    ...
    print("\n" + "=" * 80)
    print("=== 建立模型阶段：分类模型性能指标 ===")
    print("=" * 80)
    print(df_results.to_string(index=False, float_format=lambda x: f"{x:.2f}" if x < 1 else f"{x:.3f}"))
    
    save_results(df_results)
    create_metrics_table_building(df_results)
    
    # === 评估阶段 ===
    print("\n" + "=" * 80)
    print("=== 评估阶段：生成 ROC 曲线和性能指标表格 ===")
    print("=" * 80)
    create_roc_curve(roc_curves)
    create_metrics_table_evaluation(df_results)
